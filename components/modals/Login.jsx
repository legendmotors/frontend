...
export default function Login() {
  ...
  const handleSubmit = (e) => {
    ...
    if (window.clevertap) {
      // Ensure valid user data is sent
      window.clevertap.onUserLogin.push({
        "Site": {
          Identity: user.email || `user_${Date.now()}`, // Fallback to unique ID
          Email: user.email,
          Name: user.email ? user.email.split("@")[0] : "Unknown User", // Extract name from email
          Phone: user.phone || "", // Ensure it's defined
          ...
          <div className="modal-body space-y-20 pd-40">
            <div className="wrap-modal flex">
              <div className="images flex-none">
                <Image alt="images" src="/assets/images/section/login.jpg" width={380} height={640} />
              </div>
